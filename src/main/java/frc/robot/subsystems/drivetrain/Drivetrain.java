...
public class Drivetrain extends SubsystemBase {
    ...
    public void stop() {
        ...
        SwerveModuleState[] states = constants.KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    ...
    private void updateBrakeMode() {
        if (DriverStation.isEnabled()) {
            ...
            for (SwerveModule mod : swerveModules) {
                if (Math.abs(mod.getState().speedMetersPerSecond) > constants.MAX_COAST_VELOCITY_METERS_PER_SECOND) {
                    stillMoving = true;
                }
            }
